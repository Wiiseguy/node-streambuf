...
test('Buffer implementations', t => {
    ...
    t.is(
        ...
        0,
        `All read and write methods should be implemented. Missing read methods: ${
            missingReadMethods.length > 0 ? missingReadMethods : '(None)'
        }, missing write methods: ${missingWriteMethods.length > 0 ? missingWriteMethods : '(None)'}`
    )
    /* c8 ignore end */
})
...
test('correct position increase (numeric methods)', t => {
    ...
    const readMethodsOnSb = Object.getOwnPropertyNames(Object.getPrototypeOf(sb)).filter(
        m => m.startsWith('read') && !nonFixedLengthNumericReadMethods.has(m)
    )
    ...
    const writeMethodsOnSb = Object.getOwnPropertyNames(Object.getPrototypeOf(sb)).filter(
        m => m.startsWith('write') && !nonFixedLengthNumericWriteMethods.has(m)
    )
...
test('README example', t => {
    ...
    const buffer = new StreamBuffer(file)
