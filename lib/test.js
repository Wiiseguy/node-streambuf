...
test('initializers', t => {
    const sb1 = new streambuf_1.StreamBuffer(Buffer.from([1]));
    t.true(sb1 instanceof streambuf_1.StreamBuffer);
    const sb2 = streambuf_1.StreamBuffer.from(Buffer.from([1]));
    t.true(sb2 instanceof streambuf_1.StreamBuffer);
    /** @ts-ignore Intentional (wrong param) */
    let e = t.throws(_ => new streambuf_1.StreamBuffer('hello'), { instanceOf: TypeError });
    t.is(e.message, 'Not a valid Buffer or StreamBuffer');
    const buffer = Buffer.from([0, 1]);
    const sb3 = new streambuf_1.StreamBuffer(buffer);
    const sb4 = new streambuf_1.StreamBuffer(sb3);
    t.false(sb3 == sb4);
...
test('buffer access', t => {
    ...
    const sb = new streambuf_1.StreamBuffer(buffer);
    // The buffer isn't copied
    t.is(sb.buffer, buffer);
    // Access buffer length
    t.is(buffer.length, 8);
    t.is(sb.length, 8);
...
test('read part of the buffer', t => {
    ...
    const sb = new streambuf_1.StreamBuffer(buffer);
    const ssb = sb.read(3);
    t.true(ssb instanceof streambuf_1.StreamBuffer); // the read sub buffer should also be a StreamBuffer instance
    ssb.seek(1);
    t.is(ssb.readByte(), 2);
...
test('write - invalid', t => {
    ...
    sb.write('hello'), { instanceOf: TypeError });
});
test('seek / setPos', t => {
...
test('read string (unknown length)', t => {
    ...
    sb.rewind();
    t.is(sb.readString0(), 'hello');
    t.is(sb.tell(), 6);
    t.is(sb.readString0(), 'hi!');
});
...
test('writeString0 & readString0', t => {
    const buffer = Buffer.alloc(16);
    const sb = new streambuf_1.StreamBuffer(buffer);
    sb.writeString0('hello');
    sb.writeString0('world');
    t.deepEqual(buffer, [
        0x68,
        0x65,
        0x6c,
        ...
        0,
        0,
        0 // non-overwritten bytes
    ]);
    sb.rewind();
    t.is(sb.readString0(), 'hello');
    t.is(sb.readString0(), 'world');
});
...
test('Buffer implementations', t => {
    // Check if all the readX and writeX methods of Buffer are implemented in StreamBuffer
    const buffer = Buffer.alloc(8);
    const sb = new streambuf_1.StreamBuffer(buffer);
    const bufferMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(buffer));
    const sbMethods = Object.getOwnPropertyNames(Object.getPrototypeOf(sb));
    // Filter out read and write and skip the 'Uint' aliases like readUintLE (which is the same as readUIntLE)
    const bufferReadMethods = bufferMethods.filter(m => m.startsWith('read') && !m.includes('Uint'));
    const bufferWriteMethods = bufferMethods.filter(m => m.startsWith('write') && !m.includes('Uint'));
    const sbReadMethods = new Set(sbMethods.filter(m => m.startsWith('read')));
    const sbWriteMethods = new Set(sbMethods.filter(m => m.startsWith('write')));
    const missingReadMethods = bufferReadMethods.filter(m => !sbReadMethods.has(m));
    const missingWriteMethods = bufferWriteMethods.filter(m => !sbWriteMethods.has(m));
    /* c8 ignore start */
    t.is(missingReadMethods.length + missingWriteMethods.length, 0, `All read and write methods should be implemented. Missing read methods: ${missingReadMethods.length > 0 ? missingReadMethods : '(None)'}, missing write methods: ${missingWriteMethods.length > 0 ? missingWriteMethods : '(None)'}`);
    /* c8 ignore end */
});
test('correct position increase (numeric methods)', t => {
    const buffer = Buffer.from([0, 1, 2, 3, 4, 5, 6, 7]);
    const sb = new streambuf_1.StreamBuffer(buffer);
    const nonFixedLengthNumericReadMethods = new Set([
        'read',
        'readString',
        'readString7',
        'readString0',
    ...
    const readMethodsOnSb = Object.getOwnPropertyNames(Object.getPrototypeOf(sb)).filter(m => m.startsWith('read') && !nonFixedLengthNumericReadMethods.has(m));
    const nonFixedLengthNumericWriteMethods = new Set([
        'write',
        'writeString',
        'writeString7',
        'writeString0',
    ...
    const writeMethodsOnSb = Object.getOwnPropertyNames(Object.getPrototypeOf(sb)).filter(m => m.startsWith('write') && !nonFixedLengthNumericWriteMethods.has(m));
    const readMethods = [
        [sb.readInt8, 1],
        [sb.readSByte, 1],
        [sb.readInt16LE, 2],
    ...
    for (const m of readMethodsOnSb) {
    ...
    for (const m of writeMethods) {
        ...
        let writeResult = method.bind(sb)(testValue);
        t.is(writeResult, testValue, method.name);
        t.is(sb.getPos(), len, method.name);
        sb.rewind();
    }
    // Check if we missed any write methods
    for (const m of writeMethodsOnSb) {
        let check = writeMethods.find(rm => rm[0].name === m);
        t.true(check !== undefined, `Missed write method: ${m}`);
    }
});
test('correct position increase (numeric methods with byte length param)', t => {
    const buffer = Buffer.from([0x12, 0x34, 0x56, 0x78, 0x90, 0xab]);
    const sb = new streambuf_1.StreamBuffer(buffer);
    // Call each method with a byte length parameter (1-6)
    const minByteLength = 1;
    const maxByteLength = 6;
    const intResultsLE = [0x12, 0x3412, 0x563412, 0x78563412, -0x6f87a9cbee, -0x546f87a9cbee];
    // readIntLE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.readIntLE(i);
        t.is(result, intResultsLE[i - 1], `readIntLE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    // writeIntLE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.writeIntLE(intResultsLE[i - 1], i);
        t.is(result, intResultsLE[i - 1], `writeIntLE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    const intResultsBE = [0x12, 0x1234, 0x123456, 0x12345678, 0x1234567890, 0x1234567890ab];
    // readIntBE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.readIntBE(i);
        t.is(result, intResultsBE[i - 1], `readIntBE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    // writeIntBE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.writeIntBE(intResultsBE[i - 1], i);
        t.is(result, intResultsBE[i - 1], `writeIntBE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    const uintResultsLE = [0x12, 0x3412, 0x563412, 0x78563412, 0x9078563412, 0xab9078563412];
    // readUIntLE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.readUIntLE(i);
        t.is(result, uintResultsLE[i - 1], `readUIntLE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    // writeUIntLE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.writeUIntLE(uintResultsLE[i - 1], i);
        t.is(result, uintResultsLE[i - 1], `writeUIntLE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    const uintResultsBE = [0x12, 0x1234, 0x123456, 0x12345678, 0x1234567890, 0x1234567890ab];
    // readUIntBE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.readUIntBE(i);
        t.is(result, uintResultsBE[i - 1], `readUIntBE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
    // writeUIntBE
    for (let i = minByteLength; i <= maxByteLength; i++) {
        let result = sb.writeUIntBE(uintResultsBE[i - 1], i);
        t.is(result, uintResultsBE[i - 1], `writeUIntBE(${i})`);
        t.is(sb.getPos(), i);
        sb.rewind();
    }
});
...
test('readChar - utf8', t => {
    ...
    const sb = new streambuf_1.StreamBuffer(buffer);
    t.is(sb.readChar().codePointAt(0), 65533);
    t.is(sb.tell(), 1);
    sb.rewind();
...
test('README example', t => {
    ...
    const hiscoreFileSb = new streambuf_1.StreamBuffer(hiscoreFile);
    ...
    const buffer = new streambuf_1.StreamBuffer(file);
...
test('README example - readString0', t => {
    const buf = Buffer.from('Hello\0World\0');
    const sb = new streambuf_1.StreamBuffer(buf);
    t.is(sb.readString0(), 'Hello');
    t.is(sb.readString0(), 'World');
});
