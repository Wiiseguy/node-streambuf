...
test('Buffer implementations', t => {
    ...
    const sbReadMethods = sbMethods.filter(m => m.startsWith('read'));
    const sbWriteMethods = sbMethods.filter(m => m.startsWith('write'));
...
test('correct position increase (numeric methods)', t => {
    const buffer = Buffer.from([0, 1, 2, 3, 4, 5, 6, 7]);
    const sb = new streambuf_1.StreamBuffer(buffer);
    const nonFixedLengthNumericReadMethods = [
        'read',
        'readString',
        'readString7',
        'readString0',
    ...
    const readMethodsOnSb = Object.getOwnPropertyNames(Object.getPrototypeOf(sb)).filter(m => m.startsWith('read') && !nonFixedLengthNumericReadMethods.includes(m));
    const nonFixedLengthNumericWriteMethods = [
        'write',
        'writeString',
        'writeString7',
        'writeString0',
    ...
    const writeMethodsOnSb = Object.getOwnPropertyNames(Object.getPrototypeOf(sb)).filter(m => m.startsWith('write') && !nonFixedLengthNumericWriteMethods.includes(m));
    const readMethods = [
        [sb.readInt8, 1],
        [sb.readSByte, 1],
        [sb.readInt16LE, 2],
